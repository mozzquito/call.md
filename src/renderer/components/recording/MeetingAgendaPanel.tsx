...
function ChecklistItem({ text, checked, onToggle }: ChecklistItemProps) {
  return (
    <div
      className="bg-muted border border-border rounded-[10px] px-[12px] py-[8px] flex items-center gap-[12px] cursor-pointer hover:bg-muted/80 transition-colors w-full"
      onClick={onToggle}
    >
      {/* Checkbox */}
      <div
...
export function MeetingAgendaPanel({
  ...
  return (
    <div className="bg-card border border-border rounded-[16px] p-[16px] flex flex-col gap-[20px] max-h-[50%] shrink-0">
      {/* Header */}
      <div
      ...
      >
        <div className="flex-1 flex items-center gap-[8px]">
          <ClipboardCheckIcon />
          <span className="font-medium text-[15px] text-foreground">Meeting Agenda</span>
        </div>
        {isExpanded ? (
          <ChevronUp className="w-[24px] h-[24px] text-muted-foreground" />
        ) : (
          <ChevronDown className="w-[24px] h-[24px] text-muted-foreground" />
        )}
      </div>
